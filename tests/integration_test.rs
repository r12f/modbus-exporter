//! Integration tests for bus-exporter.
...
"#,
    ...
    let config = bus_exporter::config::Config::load(&path);
...
"#,
    ...
    let result = bus_exporter::config::Config::load(&path);
...
fn metric_store_publish_and_read() {
    let store = bus_exporter::metrics::MetricStore::new();
    let global = BTreeMap::from([("env".to_string(), "test".to_string())]);
    let collector_labels = BTreeMap::new();

    let metric = bus_exporter::metrics::MetricValue {
        name: "temp".to_string(),
        value: 22.5,
        metric_type: bus_exporter::metrics::MetricType::Gauge,
