use super::ModbusTcpMetricReader;
use crate::reader::modbus::{BusConnection, ModbusReader};

#[test]
fn test_tcp_client_new_not_connected() {
    let client = ModbusTcpMetricReader::new("127.0.0.1:502".to_string(), 1);
    assert!(!client.is_connected());
}

#[tokio::test]
async fn test_tcp_client_read_without_connect_fails() {
    let mut client = ModbusTcpMetricReader::new("127.0.0.1:502".to_string(), 1);
...
async fn test_tcp_client_connect_to_invalid_endpoint_fails() {
    let mut client = ModbusTcpMetricReader::new("127.0.0.1:1".to_string(), 1);
...
async fn test_tcp_client_connect_with_hostname_fails_gracefully() {
    let mut client = ModbusTcpMetricReader::new("nonexistent-host.invalid:502".to_string(), 1);
    let result = client.connect().await;
    assert!(result.is_err());
}
